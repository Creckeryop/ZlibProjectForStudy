...
for tag in tags:
    ...
    for item in library:
        ...
        print(book_id)
        cursor.execute("select keyword.id from KeyWord where keyword.Name='"+tag+"'")
        keyword_id = cursor.fetchone()[0]
        cursor.execute("Insert into BookKeyWord (Book_Id, KeyWord_Id) values ("+str(book_id)+","+str(keyword_id)+")")
